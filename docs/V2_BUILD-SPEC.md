...
**Status:** v1.7 — M1 code-complete and live in production; a second round of real-world staging testing surfaced six further bugs, all fixed and merged to `staging` but **not yet merged to `main`/production** — see §3.1.5  
...
#### 3.1.5 Post-Launch Hardening (staging, 2026-08-04 – 2026-08-05)

M1 shipped to production per v1.5 above. A second round of real-world staging testing — running actual search profiles and checking results against the live postings, not just reading logs — surfaced six further bugs. All are fixed and merged to `staging` (PRs #3–#9); **none are merged to `main`/production yet.** Documented here because they materially change M1's behavior from what v1.4/v1.5 described above.
...
**Current state:** all six fixes above are on `staging` only, pending further verification before being bundled into a PR to `main`. Item 2's `[verify][diag]` logging should be removed once it's served its purpose — either the fetch-failure question is fully settled, or enough clean runs have passed that it's no longer earning its noise in the logs.
