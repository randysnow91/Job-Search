...
**Status:** v1.8 — M1 code-complete and live in production; a second round of real-world staging testing surfaced seven further bugs, all fixed and merged to `staging` but **not yet merged to `main`/production** — see §3.1.5  
...
M1 shipped to production per v1.5 above. A second round of real-world staging testing — running actual search profiles and checking results against the live postings, not just reading logs — surfaced seven further bugs. Six are fixed and merged to `staging` (PRs #3–#9); the seventh (below) is fixed and pushed, pending its own PR. **None are merged to `main`/production yet.** Documented here because they materially change M1's behavior from what v1.4/v1.5 described above.
